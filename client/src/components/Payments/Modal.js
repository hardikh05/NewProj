...
const Modal = ({ setOpen, open, invoice }) => {
  ...
  //Crate a state to handle the payment records
    ...
    useEffect(() => {
      let totalReceived = 0
      if(invoice?.paymentRecords) {
        totalReceived = invoice.paymentRecords.reduce((sum, record) => sum + Number(record.amountPaid), 0)
      }
    ...
    useEffect(() => {
      const newTotalReceived = totalAmountReceived + Number(payment.amountPaid)
      setUpdatedInvoice({
        ...invoice, 
        status: newTotalReceived >= invoice?.total ? 'Paid' : 'Partial',
