...
const InvoiceDetails = () => {
    ...
    const totalAmountReceived = invoice?.paymentRecords?.reduce((sum, record) => sum + Number(record.amountPaid), 0) || 0

    const checkStatus = () => {
        if (totalAmountReceived === 0) return '#C70000'  // Unpaid - red
        if (totalAmountReceived >= total) return '#00A86B' // Paid - green
        return '#1976D2'  // Partial - blue
    }
  ...
  const createAndDownloadPdf = () => {
    ...
    const businessDetails = invoice?.businessDetails?.data;
  ...
  return (
    ...
    return (
        <div className={styles.PageLayout}>
           {invoice?.creator?.includes(user?.result?.sub || user?.result?._id || user?.result?.googleId) && (
