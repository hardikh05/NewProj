   ...
   const sortHistoryByDate =  paymentHistory.sort(function(a, b) {
    ...
    for(let i = 0; i < invoices.length; i++) {
        if(invoices[i].paymentRecords) {
            const paid = invoices[i].paymentRecords.reduce((sum, record) => sum + Number(record.amountPaid), 0)
            totalPaid += paid
        }
    }
    ...
    useEffect(() => {
        dispatch(getInvoicesByUser({search: user?.result?.sub || user?.result?._id || user?.result?.googleId}));
