...
const NavBar = () => {
  ...
  const [profile, setProfile] = useState(JSON.parse(localStorage.getItem('profileDetail')))

  useEffect(() => {
    setUser(JSON.parse(localStorage.getItem('profile')))
    setProfile(JSON.parse(localStorage.getItem('profileDetail')))
  }, [location])
    ...
    <li className="logo">
      <a href="/dashboard" className="nav-link">
        <span className="link-text logo-text">
          <img 
            style={{width: '50px'}} 
            src={profile?.logo || process.env.PUBLIC_URL + "/icons/icon-64x64.png"} 
            alt={profile?.businessName || "echo-invoice"} 
          />
        </span>
